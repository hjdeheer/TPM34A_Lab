...
def get_emergency_calls():
    df = pd.read_csv('source/emergency_calls_data.csv')
    return df

def get_pois():
    df = pd.read_csv('source/pois_data.csv')
    return df

def get_socio_dem(year, agg_level, lang):
    ...
    df = df.loc[(df['year'] == year) & (df['recs'] == agg)]
